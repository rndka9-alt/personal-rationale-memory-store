...
Search uses a hybrid ranking pass over vector results, lexical results, metadata filters, lifecycle state, confidence, lightweight usage signals, and explicit positive or negative usage feedback. Deprecated entries are excluded by `acceptanceState` unless explicitly requested. Accepted and reviewed memories receive trust boosts, while candidates receive only a small boost; auto-captured unreviewed candidates and memories marked `needs_revision` are penalized in search so they do not overpower reviewed guidance before cleanup. Negative feedback lowers normal search ranking but raises review queue priority as an attention signal. Returned entries include signed ranking reasons such as `vector:0.800:+4.00`, `domain-match:2:+4.00`, or `feedback:-1.50` so callers can inspect how each signal changed the score. If vector retrieval falls back to lexical retrieval, `search_rationales` returns warnings and `compose_context` includes a retrieval warnings section so the degraded path is visible without reading server logs.
