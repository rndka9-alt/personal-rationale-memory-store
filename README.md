...
Use Korean for natural-language rationale titles and bodies, note content and topics, and retrieval query or task text so stored content and searches use a consistent language. Keep code identifiers, exact search terms, proper nouns, and source conversation messages in their original form when translation would reduce precision or provenance.
...
`compose_context` retrieves broadly and returns prompt-ready rationale content without exposing ranking diagnostics or internal entry ids by default. Every agent-facing rationale id identifies the exact revision snapshot that supplied the content. Retrieved candidates must clear a vector similarity floor before packing; the floor applies to raw similarity rather than the composite score because composite scores mix trust boosts, so a boosted-but-unrelated memory could pass any composite threshold. Lexical-only matches and vector-failure fallback results are exempt so degraded retrieval stays usable. When a memory is actually included in a composed context pack, the server records a `composed` usage event for that revision and entry, and the pack ends with a feedback footer asking the client to report `applied` or `dismissed` outcomes through `rate_memory`. Passive retrieval and composition events do not increment `memory_entries.use_count` or update `memory_entries.last_used_at`; those fields are reserved for explicit positive usage feedback. Plain retrieval candidates that do not fit the context budget are not counted as used.

Use `rate_memory` after a memory is actually applied, judged helpful, judged unhelpful, or dismissed. `applied` and `dismissed` reflect the agent's own judgment after acting on retrieved context; `user_helpful` and `user_unhelpful` are reserved for explicit user reactions to an outcome the memory influenced. The tool accepts the revision snapshot id that supplied the evaluated content; the server stores that revision together with its internal entry id. `applied` and `user_helpful` events increment `use_count`, update `last_used_at`, and contribute positive search ranking signal, while `user_unhelpful` and `dismissed` preserve negative feedback without inflating usage. Ranking aggregates these events by entry across revisions, while the stored revision id preserves exact attribution for audit. Existing databases may contain historical events without revision attribution and historical `use_count` values from older `composed` events; search ranking relies on explicit feedback counts instead of raw `use_count`.
...
The pack opens with a memory index section when active index lines exist: trigger phrases promoted from recurring captures, grouped by project (`프로젝트 무관` first), each ending with its anchor memory's revision id for follow-up reads. The index is maintained asynchronously after captures, updates, deprecations, and restores — similarity is recomputed on demand from stored chunk embeddings (documents pass the 0.85 cut via their strongest chunk pair), a new document joins an existing line only through an anchor-plus-one-member quorum, and a new line is promoted when three or more unaffiliated documents cluster and one LLM call verifies they share a trigger and writes the phrase. Lines whose membership drops below three retire softly; anchors are re-elected as the cluster medoid.
...
`auto_capture_rationale` does not accept a memory `type`; MCP captures are stored as `rationale`. The classification never fed search or ranking, so choosing one was pure caller overhead. Internal capture paths may still set a type, `principle` remains reserved for promotion from accepted rationale, and existing typed entries (`known_failure`, `preference`, `convention`, `constraint`) stay stored and searchable.
